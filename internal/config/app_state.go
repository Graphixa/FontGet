...
// LoadAppState loads the application state from file
func LoadAppState() (*UserConfig, error) {
...
// SaveAppState saves the application state to file
func SaveAppState(config *UserConfig) error {
	configPath, err := GetConfigPath()
	if err != nil {
		return err
	}
