...
func pickArchiveCandidates(paths []string, prefix, fontName, fontID string) []string {
	...
	// Nerd Fonts: package mode. The manifest already selected the upstream archive URL.
	// Install every supported desktop font in that package; do not match catalogue names
	// to filename family tokens (OFL remaps like Cascadia→Caskaydia would otherwise fail).
	if isNerdPackageSource(p) {
		output.GetDebug().State("archive pick: branch=known:nerd package mode count=%d fontName=%q fontID=%q",
			len(paths), fontName, fontID)
		return paths
	}
...
// Nerd Fonts use package mode in pickArchiveCandidates (all desktop fonts in the archive).
