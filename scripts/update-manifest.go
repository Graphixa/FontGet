...
func main() {
	...
	for family, variants := range fontFamilies {
		...
		if metadataPath != "" {
			metadata = fetchMetadata(rawContentBaseURL + metadataPath)
		}
		...
		if metadata.Category != "" {
			categories = append(categories, metadata.Category)
		}
...
func fetchMetadata(url string) FontMetadata {
	resp, err := http.Get(url)
	if err != nil {
		fmt.Printf("Warning: Failed to fetch metadata: %v\n", err)
		return FontMetadata{}
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		fmt.Printf("Warning: Failed to fetch metadata: HTTP %d\n", resp.StatusCode)
		return FontMetadata{}
	}
